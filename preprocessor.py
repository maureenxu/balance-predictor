...
async def preprocess(request: Request):
    ...
    input_path = os.path.join(config["DEFAULT"]["base_path"], config["PREPROCESSING"]["input_path"])
    ...
    utils.pickle_dump_output(config["DEFAULT"]['base_path'], config["PREPROCESSING"]["output_path"], df_output)
